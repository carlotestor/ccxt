...
none of them is a judgement call):
  ...
  6. policy              watch* (live cache identity), parse* (inputs to further
                         transpiled logic), *Ws (WS tier), and the RUNTIME_SHAPE
                         opt-outs whose declared List<T> is a dict at runtime.
...
CONV = re.compile(r'Object res = Helpers\.joinUnwrapped\(super\.(\w+)\([^;]*\)\);\s*\n\s*'
                  r'return (?:new (\w+)\(res\)|toTypedList\(res, (\w+)::new\)|Helpers\.joinTyped|(.+?));')
CONV_TYPED = re.compile(r'return Helpers\.joinTyped\(super\.(\w+)\([^;]*\)\);')
TYPED_SYNC_SIG = re.compile(r'^\s*public (?:java\.util\.)?(List<(\w+)>|(\w+)) (\w+)\((.*)\) \{\s*$')


def tier_files(tier):
    if tier == 'crypto':
        return ([os.path.join(ROOT, 'BaseExchange.java'), os.path.join(ROOT, 'Exchange.java')]
                + [p for p in glob.glob(ROOT + '/exchanges/**/*Core.java', recursive=True) if '/prediction/' not in p])
    ...
    return ([os.path.join(ROOT, 'BaseExchange.java'), os.path.join(ROOT, 'PredictionExchange.java')]
            + sorted(glob.glob(ROOT + '/exchanges/prediction/*Core.java')))


def wrapper_files(tier):
    if tier == 'crypto':
        return [p for p in glob.glob(ROOT + '/exchanges/**/*.java', recursive=True)
                if not p.endswith('Core.java') and '/prediction/' not in p]
    return [p for p in glob.glob(ROOT + '/exchanges/prediction/*.java') if not p.endswith('Core.java')]
...
def compute(verbose=False):
    ...
    for tier in ('crypto', 'prediction'):
        ...
        for name in sorted(fams):
            fs = fams[name]
            why = None
            if name.startswith('watch') or name.startswith('parse') or name.endswith('Ws'):
                why = 'policy: watch*/parse*/*Ws are out of scope'
